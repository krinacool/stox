...
def custom_admin_context(request):
    ...
    transactions = Transaction.objects.all().filter(status='COMPLETED',datetime__date=today)
    ...
    for x in orders:
        brokerage_collected = brokerage_collected + x.charges
    total_deposit = 0
    total_withdraw = 0
    for x in transactions:
        if x.transaction_type == 'DEPOSIT':
            total_deposit = total_deposit + x.amount
        else:
            total_withdraw = total_withdraw + x.amount
    for x in all_users:
    ...
    context={
        ...
        "total_users":all_users.__len__(),
        "api_users":all_users.filter(api_orders=True).__len__(),
        "chart_data": as_json,
